...
class LessonService:
    ...
    def build_hardcoded_lesson(self):
        ...
        node2 = LessonNode(id="node2", steps_DLL_root_id="step2")
        # 6. Build dicts
        elements = {e.id: e for e in [el1, el2]}
        commands = {c.id: c for c in [cmd1, cmd2]}
        command_groups = {cg.id: cg for cg in [cg1, cg2]}
        steps = {s.id: s for s in [step1, step2]}
